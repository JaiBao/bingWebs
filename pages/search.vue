              ...
              <q-list dense>
                ...
                <q-item>
                  ...
                  <q-item-section side>
                    <q-item-label
                      caption
                      class="fz-0875r isLink pointer"
                      @click="fetchOrderDetails(props.row.orderID, props.row.personal_name, props.row.mobile, formatAddress2(props.row))">
                      {{ props.row.code }}
                    </q-item-label>
                  </q-item-section>
                </q-item>
      ...
      <div class="cardContainer2">
        <q-card class="orderContainer" v-for="(order, index) in structuredMeals" :key="index">
          ...
          <div class="items">
            <!-- 顯示主餐和副主餐的組合 -->
            <div class="item">
              <span>{{ order.side_meals.mealCombo }}</span>
            </div>
            <!-- 分隔線 -->
...
const fetchOrderDetails = async (orderID, personalName, mobile, address) => {
  try {
  ...
  return orderDetails.value.map(order => {
    ...
    order.side_meals.forEach(meal => {
      ...
      if (meal.name === '主餐' || meal.name === '6吋潤餅') {
        combo += `${mealName} x ${meal.quantity}`
        // } else if (meal.name === '副主餐') {
        //   if (combo.length > 0) combo += ' + '
        //   combo += `${meal.value} x ${meal.quantity}`
      } else if (meal.name === '飲料') {
        categories.beverages.push({ value: mealName, quantity: meal.quantity, id: meal.id })
      }
    })

    categories.mealCombo = combo

    // 返回此組的完整結構
    return {
      main_meal: {
        name: order.main_meal.name,
